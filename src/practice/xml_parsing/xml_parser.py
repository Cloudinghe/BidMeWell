...
from practice import standard_formulas
from practice.redeal import Evaluator
from practice.redeal.global_defs import Strain
from practice.xml_parsing.conditions import SimpleCondition
from practice.xml_parsing.conditions import ShapeConditionFactory
from practice.xml_parsing.conditions import AndCondition, OrCondition
...
def _eval_node(node):
    if isinstance(node, ast.Num):
        ...
        return AST_OPERATOR_MAP[type(node.op)](
...
class Bid:
    ...
    def _get_suit(self) -> str:
        ...
        # 特殊叫品：加倍(x)和再加倍(xx)
        if value_lower == "x" or value_lower == "xx":
            return None
        if value_lower == "pass":
            return None
        if value_lower == "p":
...
def _get_min_max_for_method(
    ...
    assert not (
        result[0] in {default_min, absolute_min}
        and result[1] in {default_max, absolute_max}
    ), xml_method.tag
...
def _parse_formula_for_condition(formula):
    ...
    def _evaluate_expression(expression: str, hand_shape) -> int:
        ...
        evaluated_expression = (
            expression.replace("s", str(s))
            .replace("h", str(h))
            .replace("d", str(d))
            .replace("c", str(c))
        )
        assert SAFE_EXPRESSION.match(evaluated_expression), evaluated_expression
...
def _get_formula_module(xml_root, current_directory):
    try:
        ...
        formula_module_location = os.path.join(current_directory, formula_module_name)
        spec = importlib.util.spec_from_file_location(
            "bridge_formulas", formula_module_location
        )
        ...
        print(
            f"Warning: formula module could not be found at {formula_module_location}"
        )
        formula_module = None
...
def _get_shape_conditions(xml_condition):
    ...
    for shape in shapes:
        ...
        if type_ == "shape":
            shape_condition = ShapeConditionFactory.create_shape_condition(shape.text)
        elif type_ == "general":
            shape_condition = ShapeConditionFactory.create_general_shape_condition(
                shape.text
            )
        elif type_ == "formula":
            formula = shape.text
            ...
            minimum, maximum = _get_min_max_for_method(
                shape, absolute_min=0, absolute_max=13
            )

            shape_condition = ShapeConditionFactory.create_suit_length_condition(
            ...
            accept = _parse_formula_for_condition(formula)
            shape_condition = SimpleCondition(accept, f"Formula: {formula}")
        else:
...
class XmlReaderForFile:
    ...
    def _get_formula(self, method_name):
        try:
            ...
            try:
                ...
                raise NotImplementedError(f"{method_name} not defined in {location}.")
    ...
    def _get_evaluation_conditions(self, xml_condition):
        ...
        for method in evaluation:
            ...
            if method.tag == "hcp":
                evaluation_condition = EvaluationCondition(self.hcp, minimum, maximum)
            elif method.tag == "tricks":
                tricks = self._get_formula("tricks")

                evaluation_condition = EvaluationCondition(tricks, minimum, maximum)
    ...
    def _define_bid(self, xml_bid) -> Bid:
        ...
        for xml_condition in xml_conditions:
            evaluation_conditions = self._get_evaluation_conditions(xml_condition)
            ...
            if type_ == "include":
                ...
                raise NotImplementedError(type_, "Expected 'include' or 'exclude'")
    ...
    def get_bids_from_xml(self):
        result = {}
        # 支持新的 XML 结构：根元素是 biddingSystem，子元素是 openingBids/competitiveBids/slamBids
        # 也支持旧的 XML 结构：根元素直接是 openingBids
        if self._root.tag == "biddingSystem":
            # 新结构：从 openingBids 子元素开始解析
            opening_bids = self._root.find("openingBids")
            if opening_bids is not None:
                xml_bids = list(opening_bids)
            else:
                xml_bids = list(self._root)
        else:
            # 旧结构：根元素就是 openingBids
            xml_bids = list(self._root)

        for xml_bid in xml_bids:
            try:
