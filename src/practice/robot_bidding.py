...
from practice.xml_parsing.xml_parser import Bid
from practice.bridge_parser import parse_with_quit, ParseResults
...
class BiddingProgram:
    ...
    # is dealer.
    _dealer_map = {1: Players.North, 2: Players.East, 3: Players.South, 0: Players.West}
    _pass = Bid("P", "Pass", [])
    ...
    def get_validated_input(
        self, message, valid_formats, help_message=None, exclude_settings=False, tries=0
    ) -> (str, ParseResults):
    ...
    def bid(self):
        ...
        if current_bidder in {self.Players.East, self.Players.West}:
            ...
            current_bidder == self.Players.South
            and self._mode == self.ProgramMode.Default
        ):
        ...
        if (next_bid != self._pass) and self._settings["display_meaning_of_bids"]:
    ...
    def _program_bid(self, current_hand):
        ...
        if len(self.bidding_sequence) >= 2:
            ...
            if current_bid != self._pass:
                ...
                potential_bids = [
                    bid
                    for bid in current_bid.children.values()
                    if bid.accept(current_hand)
                ]

        if potential_bids is None:
            potential_bids = [
                bid for bid in self._root.values() if bid.accept(current_hand)
            ]
    ...
    def _user_bid(self):
        ...
        while bid is None:
            ...
            selected, _ = self.get_validated_input(
                ...
                help_message=(
                    "Enter a bid from one of the potential bids "
    ...
    def edit_settings(self):
        ...
        while result != ParseResults.Back:
            message = (
                "Enter a key to edit the value for that key, or "
                "'back' to exit the settings editor.\n"
                "'Exit' will end the program.\n"
            )
            input_, result = self.get_validated_input(
                message,
                {ParseResults.Back, ParseResults.Integer},
            ...
            if key == "mode":
                input_ = input(
                    f"Do you wish to change the mode of the program"
                ...
                input_, result = self.get_validated_input(
                    f"Do you wish to change {key} from {self._settings[key]}? (y/n)",
    ...
    def get_contract(self, bidding_sequence=None):
        ...
        player_map = {
            self.Players.North: "N",
            self.Players.East: "E",
            self.Players.South: "S",
            self.Players.West: "W",
        }
